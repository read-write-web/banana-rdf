 ...
 */
class NTriplesWriter[Rdf <: RDF](implicit
  val ops: RDFOps[Rdf]
) extends RDFWriter[Rdf, Try, NTriples] {
  ...
  def node2Str(node: Rdf#Node, base: Rdf#URI): String = ops.foldNode(node)(
    uri => s"""<${asString(uri, base)}>""",
    { case ops.BNode(id) => "_:" + id },
    _ match {
      case ops.Literal((string, _, Some(lang)))  => s""""$string"@$lang"""
      case ops.Literal((string, datatype, None)) => s""""$string"^^<${asString(datatype, base)}>"""
    }
  )
